...
import {createDB, retrieveData, storeData, clearSensitiveTables} from "~/composables/useIndexedDB";
...
export const useAuthStore = defineStore('auth', () => {
  ...
  function reset() {
    ...
    clearSensitiveTables().catch((err) => console.log(err))
  ...
  const isAuthenticated = async () => {
    ...
    try {
      ...
      if (!await checkServerAccess()) {
        ...
        currentUser.value = (await retrieveData('users', parseInt(tokenUserID))).data.data;
      } else {
