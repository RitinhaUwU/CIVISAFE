...
class IncidentPCORequest extends FormRequest
{
    public function rules(): array
    {
        ...
        return [
            'function_pco' => [$is_patch ? 'sometimes' : 'required', 'string', 'min:1'],
            'resp_pco' => [$is_patch ? 'sometimes' : 'required', 'string', 'min:1'],
            'category_pco' => [$is_patch ? 'sometimes' : 'required', 'string', 'min:1'],
            'contact1_pco' => [$is_patch ? 'sometimes' : 'required', 'regex:/^\+?[0-9]+(?: [0-9]+)*$/'],
            ...
            'start_pco_datetime' => [$is_patch ? 'sometimes' : 'required', 'date'],
